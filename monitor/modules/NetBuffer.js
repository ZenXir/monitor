...
var NetBuffer = function (bufferLength) {
    ...
    this.push = function(buffer, offset, len){
        logger.warn("---------------->:" + buffer.toString('hex').toUpperCase());
    ...
    this.pop = function() {
        ...
        do {
            ...
            if(_buffer.length - _readOffset >= _headLen){
                //读取包头，即传过来的字符串的长度
                msgheader.peek(_buffer.slice(_readOffset, _readOffset+_headLen));
                logger.warn('---------------> peek msgheader:' + JSON.stringify(msgheader));
                if (msgheader._size > GlobalProto.Instance().maxMsgLength() || msgheader._size < msgheader.lengthIncludeInHeader()) {
                    // 返回消息错误使断开连接 : 消息大小超出最大范围 或 消息大小竟然小于消息头大小
            ...
            // 读取包头外的剩余数据
            let leftlen = msgheader._size - msgheader.lengthIncludeInHeader();
            ...
            //msgheader._type = databuff['readUInt' + (8*2) + 'BE'](0);
            //msgheader.flag1 = databuff['readUInt' + (8*4) + 'BE'](2);
            //databuff = databuff.slice(6);
